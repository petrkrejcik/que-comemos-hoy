...
export const ItemDetail = (props) => {
  ...
  const { refetch: save, isLoading: isSaving } = useCrud(
    ...
    props.handleSave(getValues(), props.id)
  );
  const { refetch: remove, isLoading: isRemoving } = useCrud(`${props.queryKey}Remove`, props.handleRemove(props.id));
  ...
  React.useEffect(() => {
    setHeader({
      ...
      ...(props.id && { menu: [{ title: 'Remove', action: remove }] }),
    });
    if (!props.active) {
      // reset to empty
      reset();
    }
  }, [save, setHeader, props.active, handleSubmit, reset, history.goBack, remove, props.id]);
